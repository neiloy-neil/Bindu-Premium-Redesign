...
export default function ProductGallery({ images, productName = "Product" }: { images: any[]; productName?: string }) {
  ...
  const activeImage = activeItem?.url || "/placeholder.svg"
  ...
  return (
    ...
    <div className="flex flex-col md:flex-row-reverse gap-4 md:gap-6 sticky top-20">
      ...
      {/* Main Viewer */}
      <div className="w-full flex-1 relative bg-bindu-light-grey overflow-hidden group">
        <div className="aspect-[3/4] w-full relative h-[60vh] md:h-[75vh] max-h-[800px]">
          {isActiveVideo && playingVideo && embed ? (
            ...
            <>
              <Image
                ...
                className="object-contain md:object-contain transition-transform duration-700 ease-out group-hover:scale-105 cursor-zoom-in"
                priority
              ...
              {isActiveVideo && (
                ...
                >
                  <div className="w-16 h-16 rounded-full bg-white/90 flex items-center justify-center shadow-lg">
                    <Play className="w-7 h-7 text-bindu-navy ml-1" fill="currentColor" />
                  </div>
                </button>
              )}
            </>
        ...
        {images.length > 1 && (
          <>
            <button onClick={prevImage} className="md:hidden absolute left-4 top-1/2 -translate-y-1/2 p-2 bg-white/80 backdrop-blur-sm rounded-full shadow-sm">
              <ChevronLeft className="w-5 h-5 text-bindu-navy" />
            </button>
            <button onClick={nextImage} className="md:hidden absolute right-4 top-1/2 -translate-y-1/2 p-2 bg-white/80 backdrop-blur-sm rounded-full shadow-sm">
              <ChevronRight className="w-5 h-5 text-bindu-navy" />
            </button>
          </>
        )}
        ...
        {!isActiveVideo && (
          <button
            ...
            className="hidden md:flex absolute bottom-4 right-4 p-3 bg-white/90 backdrop-blur-sm text-bindu-navy rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-bindu-orange hover:text-white shadow-sm"
          >
      ...
      <div className="flex md:flex-col gap-3 overflow-x-auto md:w-20 lg:w-24 shrink-0 pb-2 md:pb-0 hide-scrollbar snap-x">
        {images.map((img, idx) => (
          <button
            ...
            className={`relative aspect-[3/4] w-20 md:w-full overflow-hidden transition-all snap-center rounded-sm ${activeIndex === idx ? "ring-1 ring-bindu-navy ring-offset-2 opacity-100" : "opacity-60 hover:opacity-100"}`}
          >
        ...
        {images.length === 0 && (
          <div className="aspect-[3/4] w-20 md:w-full bg-bindu-light-grey rounded-sm" />
        )}
      </div>
    ...
    {lightbox && (
      ...
      >
        ...
        >
          <Image
            src={images[lbIndex]?.url || "/placeholder.svg"}
            alt="Product"
            fill
            sizes="100vw"
