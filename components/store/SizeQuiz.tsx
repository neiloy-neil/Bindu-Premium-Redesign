...
export default function SizeQuiz({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-bindu-border p-6 max-w-md w-full relative">
      {onClose && (
        <button onClick={onClose} className="absolute top-4 right-4 text-bindu-text-muted hover:text-bindu-black">
          <X className="w-5 h-5" />
        </button>
      )}

      <div className="flex items-center gap-2 mb-6">
        <Ruler className="w-5 h-5 text-bindu-cyan" />
        <h2 className="font-heading font-bold text-lg">Find Your Size</h2>
      </div>

      {result ? (
        <div className="text-center py-4">
          <p className="text-sm text-bindu-text-muted mb-3">Based on your answers, we recommend</p>
          <div className="w-24 h-24 rounded-full bg-bindu-black text-white flex items-center justify-center text-3xl font-bold mx-auto mb-4">
            {result}
          </div>
          <p className="text-xs text-bindu-text-muted mb-6">
            This is a general recommendation. Check the size guide for exact measurements.
          </p>
          <div className="flex gap-3 justify-center">
            <button
              onClick={() => { setStep(0); setAnswers({}); setResult(null) }}
              className="text-xs border border-bindu-border rounded-full px-4 py-2 hover:bg-bindu-muted transition-colors"
            >
              Retake quiz
            </button>
            {onClose && (
              <button
                onClick={onClose}
                className="text-xs bg-bindu-black text-white rounded-full px-4 py-2 hover:bg-bindu-cyan transition-colors"
              >
                Got it!
              </button>
            )}
        ...
        <>
          <div className="flex gap-1 mb-6">
            {QUESTIONS.map((_, i) => (
              <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i <= step ? "bg-bindu-black" : "bg-bindu-muted"}`} />
            ))}
          </div>
          ...
          <div className="space-y-2">
            {current.options.map(opt => (
              <button
                ...
                className="w-full text-left px-4 py-3 rounded-xl border border-bindu-border hover:border-bindu-black hover:bg-bindu-muted/30 transition-all text-sm flex items-center justify-between group"
              >
                {opt.label}
                <ChevronRight className="w-4 h-4 text-bindu-text-muted group-hover:text-bindu-black transition-colors" />
              </button>
            ))}
          </div>
          <p className="text-xs text-bindu-text-muted mt-4 text-center">
