  ...
  return (
    <div className="flex items-center gap-4">
      ...
      <select
        ...
        className="border border-bindu-border-grey bg-white text-bindu-navy font-medium text-sm px-4 py-2 rounded-full focus:ring-1 focus:ring-bindu-navy focus:border-bindu-navy outline-none cursor-pointer uppercase tracking-widest text-xs"
      >
      ...
      {/* Grid / List toggle */}
      <div className="hidden md:flex items-center gap-1 bg-bindu-light-grey border border-bindu-border-grey p-1 rounded-full text-bindu-text-muted">
        <button
          onClick={() => router.push(buildUrl({ view: "grid" }))}
          className={`p-1.5 rounded-full transition-colors ${isGrid ? "bg-white shadow-sm text-bindu-navy" : "hover:text-bindu-navy"}`}
          title="Grid view"
        >
