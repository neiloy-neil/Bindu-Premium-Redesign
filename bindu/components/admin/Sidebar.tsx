...
import {
  ...
  Search, ChevronDown, Mail, MessageSquare, HelpCircle, Filter, Sparkles,
} from "lucide-react"

type NavItem = { href: string; icon: any; label: string }
type NavGroup = { label: string; icon: any; items: NavItem[] }

// Always-visible quick-access pins — the 5 most-visited pages, one click regardless of group state
...
const navGroups: NavGroup[] = [
  {
    ...
    items: [
      { href: "/admin/products",          icon: ShoppingBag, label: "Products" },
      { href: "/admin/drops",             icon: Calendar,    label: "Drops" },
  ...
  },
  {
    ...
    icon: Globe,
    items: [
      { href: "/admin/episodes", icon: Sparkles,      label: "Episodes" },
...
export function Sidebar() {
  ...
  return (
    <nav className="flex flex-col gap-0.5 px-3">
      ...
      {/* Filtered flat results (search active) */}
      {filtered && (
      ...
      {/* Default view: pinned strip + collapsible groups */}
      {!filtered && (
        <div className="flex flex-col gap-0">
          {/* Pinned quick-access — always visible, one click regardless of group state */}
          <div className="flex flex-col gap-0.5 mb-1">
            <p className="px-3 pt-1 pb-0.5 text-[10px] font-semibold uppercase tracking-wider text-slate-600">
              Quick Access
            </p>
            {PINNED.map((item) => (
              <Link
