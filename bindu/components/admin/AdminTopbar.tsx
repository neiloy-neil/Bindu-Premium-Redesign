...
import { Search, PlusCircle, ShoppingCart, Bell, Menu, Package2, ChevronDown, LogOut, Settings, User, RotateCcw } from "lucide-react"
...
export default function AdminTopbar({ email }: { email: string }) {
  ...
  const [notifCounts, setNotifCounts] = useState({ pendingOrders: 0, lowStock: 0, pendingReturns: 0 })
  ...
  // Fetch notification counts on mount and whenever the window regains focus
  const fetchNotifs = async () => {
    try {
      const res = await fetch("/api/admin/notifications")
      if (res.ok) setNotifCounts(await res.json())
    } catch {}
  }
  ...
  const totalAlerts = notifCounts.pendingOrders + notifCounts.lowStock + notifCounts.pendingReturns
      ...
      <div className="flex items-center gap-2 ml-auto">
        ...
        <div ref={notifRef} className="relative">
          <button
            ...
            className="relative flex h-8 w-8 items-center justify-center rounded-lg hover:bg-muted transition-colors text-muted-foreground hover:text-foreground"
            aria-label="Notifications"
          >
            <Bell className="h-4 w-4" />
            {totalAlerts > 0 && (
              <span className="absolute -top-0.5 -right-0.5 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-red-500 px-0.5 text-[10px] font-bold text-white leading-none">
                {totalAlerts > 99 ? "99+" : totalAlerts}
              </span>
            )}
          </button>
          {notifOpen && (
            <div className="absolute right-0 top-full mt-1 w-80 rounded-xl border bg-popover shadow-lg overflow-hidden z-50">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {notifCounts.pendingOrders} order{notifCounts.pendingOrders !== 1 ? "s" : ""} awaiting confirmation
                        </p>
                      </div>
                      <span className="text-base font-bold text-blue-600 shrink-0">{notifCounts.pendingOrders}</span>
                    </Link>
                  )}
                  {notifCounts.lowStock > 0 && (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {notifCounts.lowStock} variant{notifCounts.lowStock !== 1 ? "s" : ""} running low (≤5 units)
                        </p>
                      </div>
                      <span className="text-base font-bold text-amber-600 shrink-0">{notifCounts.lowStock}</span>
                    </Link>
                  )}
                  {notifCounts.pendingReturns > 0 && (
                    <Link
                      href="/admin/returns"
                      onClick={() => setNotifOpen(false)}
                      className="flex items-center gap-3 px-4 py-3 hover:bg-muted transition-colors"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {notifCounts.pendingReturns} return{notifCounts.pendingReturns !== 1 ? "s" : ""} awaiting review
                        </p>
                      </div>
                      <span className="text-base font-bold text-red-600 shrink-0">{notifCounts.pendingReturns}</span>
                    </Link>
                  )}
                </div>
              )}
