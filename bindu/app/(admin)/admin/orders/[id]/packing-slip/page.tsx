...
export default async function PackingSlipPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const storeName = map.store_name || "Bindu Premium"
    ...
    <html>
      ...
      <body>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 16, paddingTop: 12, borderTop: "1px dashed #ccc" }}>
          <img
            src={`https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=${encodeURIComponent(`${process.env.NEXT_PUBLIC_SITE_URL || "https://www.bindupremiumbd.com"}/track/${order.orderNumber}`)}`}
            alt="Track order QR"
            width={80}
            height={80}
            style={{ flexShrink: 0 }}
