...
export default async function CheckoutPage() {
  ...
  const [settings, checkoutFields, loyaltyData, creditData, shippingZones] = await Promise.all([
    prisma.setting.findMany({
      where: { key: { in: ["free_shipping_above", "shipping_charge", "enabled_payment_methods", "tax_enabled", "tax_rate", "tax_label", "gift_wrap_enabled", "gift_wrap_charge", "loyalty_points_per_taka", "points_redemption_rate"] } },
    }).catch(() => []),
  ...
  const loyaltyRedemptionRate = Number(map.points_redemption_rate || 100)
  ...
  return (
    <div className="bg-bindu-light-grey min-h-screen pt-8 pb-24 animate-in fade-in duration-500">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">

        <div className="flex flex-col items-center justify-center mb-10 text-center">
          <h1 className="text-3xl md:text-4xl font-heading font-bold text-bindu-navy mb-2 uppercase tracking-tight">Checkout</h1>
          <p className="text-xs text-bindu-text-muted flex items-center gap-1 uppercase tracking-widest font-bold">
            <ShieldCheck className="w-4 h-4 text-bindu-orange" /> Secure 256-bit SSL Encryption
          </p>
        </div>
