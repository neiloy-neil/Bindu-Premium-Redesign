...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    } else if (campaign.recipientType === "MEMBERS") {
      const subs = await prisma.membershipSubscription.findMany({
        where: { status: "ACTIVE" },
        select: { userId: true },
      })
      const memberIds = subs.map(s => s.userId)
      users = await prisma.user.findMany({ where: { id: { in: memberIds } }, select: { email: true } })
    } else if (campaign.recipientType.startsWith("TAG:")) {
      const tag = campaign.recipientType.slice(4)
      const tagged = await prisma.customerTag.findMany({ where: { tag }, include: { user: { select: { email: true } } } })
      users = tagged.map(t => ({ email: t.user.email }))
    }
