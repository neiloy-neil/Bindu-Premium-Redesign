...
export default async function InvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const storeName = settingsMap.store_name || "Bindu Premium"
  const supportEmail = settingsMap.support_email || "support@bindupremium.com.bd"
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-12 print:p-0">
      ...
      <div className="border border-bindu-border rounded-2xl p-8 print:border-0 print:rounded-none print:p-0">
        <div className="flex items-start justify-between mb-8 pb-8 border-b border-bindu-border">
          <div>
            <h2 className="text-2xl font-heading font-bold">{storeName}</h2>
            <p className="text-xs text-bindu-text-muted mt-1">{supportEmail}</p>
          </div>
          <div className="text-right">
            <p className="text-xs uppercase tracking-widest text-bindu-text-muted font-bold">Invoice</p>
            <p className="font-mono font-bold">{order.orderNumber}</p>
            <p className="text-xs text-bindu-text-muted mt-1">{new Date(order.createdAt).toLocaleDateString("en-BD")}</p>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-8 mb-8 text-sm">
          <div>
            <p className="text-xs uppercase tracking-widest text-bindu-text-muted font-bold mb-2">Billed To</p>
            <p className="font-medium">{order.shippingName}</p>
            <p className="text-bindu-text-muted">{order.shippingPhone}</p>
            <p className="text-bindu-text-muted">{order.shippingAddress}</p>
            <p className="text-bindu-text-muted">{order.shippingArea}, {order.shippingDistrict}, {order.shippingDivision}</p>
          </div>
          <div>
            <p className="text-xs uppercase tracking-widest text-bindu-text-muted font-bold mb-2">Payment</p>
            <p className="font-medium">{order.paymentMethod === "COD" ? "Cash on Delivery" : order.paymentMethod}</p>
            <p className="text-bindu-text-muted">Status: {order.paymentStatus}</p>
        ...
        <table className="w-full text-sm mb-8">
          <thead>
            <tr className="border-b border-bindu-border text-xs uppercase tracking-widest text-bindu-text-muted">
          ...
          <tbody>
            {order.items.map((item) => (
              <tr key={item.id} className="border-b border-bindu-border/50">
                <td className="py-3">
        ...
        <div className="flex justify-end">
          <div className="w-56 space-y-2 text-sm">
            ...
            <div className="flex justify-between font-bold text-base pt-2 border-t border-bindu-border">
              <span>Total</span>
