  ...
  return (
    <div className="space-y-4">
      {bundle.items.map(item => {
        ...
        return (
          <div key={item.id} className="space-y-2">
            <p className="text-xs font-bold uppercase tracking-widest text-bindu-text-muted">{item.product.name}</p>
            <div className="flex flex-wrap gap-2">
              {variants.map(v => (
                <button
                  key={v.id}
                  onClick={() => selectVariant(item.product.id, v.id)}
                  className={`px-3 py-1.5 text-xs border rounded transition-all ${selectedVariants[item.product.id] === v.id ? "bg-bindu-black text-white border-bindu-black" : "border-bindu-border hover:border-bindu-black"}`}
                >
                  {v.size} / {v.color}
                </button>
              ))}
              {variants.length === 0 && <p className="text-xs text-bindu-error">Out of stock</p>}
            </div>
          </div>
        )
      })}
      ...
      <button
        ...
        className="w-full py-4 bg-bindu-black text-white font-bold uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-bindu-cyan transition-colors text-sm mt-2"
      >
