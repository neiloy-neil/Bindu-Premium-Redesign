...
export default function AffiliatesClient({ data }: { data: Affiliate[] }) {
  ...
  const [editForm, setEditForm] = useState({ name: "", email: "", commissionType: "PERCENTAGE", commissionValue: "10", isActive: true })
  ...
  function openEdit(a: Affiliate) {
    setEditTarget(a)
    setEditForm({ name: a.name, email: a.email, commissionType: a.commissionType, commissionValue: String(a.commissionValue), isActive: a.isActive })
  }
  ...
  async function handleEdit(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/admin/affiliates/${editTarget.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editForm) })
      if (!res.ok) throw new Error((await res.json()).error)
      ...
      <Dialog open={!!editTarget} onOpenChange={(v) => { if (!v) setEditTarget(null) }}>
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleEdit} className="space-y-4 mt-2">
            <div><label className="text-sm font-medium">Full Name</label><Input value={editForm.name} onChange={e => setEditForm({ ...editForm, name: e.target.value })} required /></div>
            <div><label className="text-sm font-medium">Email</label><Input type="email" value={editForm.email} onChange={e => setEditForm({ ...editForm, email: e.target.value })} required /></div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium">Commission Type</label>
                <Select value={editForm.commissionType} onValueChange={v => setEditForm({ ...editForm, commissionType: v ?? editForm.commissionType })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="PERCENTAGE">Percentage (%)</SelectItem>
                    <SelectItem value="FLAT">Flat Amount (৳)</SelectItem>
              ...
              <div>
                <label className="text-sm font-medium">Commission Value</label>
                <Input type="number" value={editForm.commissionValue} onChange={e => setEditForm({ ...editForm, commissionValue: e.target.value })} />
              </div>
            </div>
