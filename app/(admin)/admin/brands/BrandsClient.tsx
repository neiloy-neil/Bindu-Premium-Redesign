...
function emptyForm() {
  return { name: "", slug: "", logo: "", banner: "", description: "", website: "", isActive: true, sortOrder: 0 }
}
...
export default function BrandsClient({ data }: { data: Brand[] }) {
  ...
  function openEdit(b: Brand) {
    setEditing(b)
    setForm({ name: b.name, slug: b.slug, logo: b.logo || "", banner: (b as any).banner || "", description: b.description || "", website: b.website || "", isActive: b.isActive, sortOrder: b.sortOrder })
    setOpen(true)
  }
        ...
        <DialogContent>
          ...
          <div className="space-y-3 mt-2">
            ...
            <div>
              ...
              <ImageUploadInput value={form.logo} onChange={(url) => setForm(f => ({ ...f, logo: url }))} />
            </div>
            <div>
              <label className="text-sm font-medium">Banner Image (optional)</label>
              <ImageUploadInput value={(form as any).banner || ""} onChange={(url) => setForm(f => ({ ...f, banner: url }))} />
            </div>
            <div>
